/* eslint-disable max-len, camelcase, n */
/* global Gun */
/*
...
*/
const debug = require('debug')('dweb-transports:gun');
...
class TransportGUN extends Transport {
  ...
  connection(url) {
    ...
    */
    ...
    const patharray = url.pathname.split('/');   // [ 'gun', database, table ] but could be arbitrary length path
    patharray.shift();  // Loose leading ""
    patharray.shift();    // Loose "gun"
    debug('path=', patharray);
  ...
  static setup0(options) {
    ...
    */
    const combinedoptions = Transport.mergeoptions(defaultoptions, options.gun);
    debug('options %o', combinedoptions);
    const t = new TransportGUN(combinedoptions);     // Note doesnt start IPFS or OrbitDB
  ...
  async p_setup1() {
    ...
    */
    try {
      ...
      debug('%s failed to start %o', this.name, err);
  ...
  async p_rawfetch(url) {
    ...
    const g = this.connection(url); // Goes all the way to the key
    const val = await this._p_once(g);
    // g.on((data)=>debug("Got late result of: %o", data)); // Checking for bug in GUN issue#586 - ignoring result
    if (!val) throw new TransportError('GUN unable to retrieve: ' + url.href);  // WORKAROUND-GUN-ERRORS - gun doesnt throw errors when it cant find something
    const o = typeof val === 'string' ? JSON.parse(val) : val;  // This looks like it is sync (see same code on p_get and p_rawfetch)
  ...
  async p_rawadd(url, sig) {
    ...
    */
    ...
    if (!(url && sig.urls.length && sig.signature && sig.signedby.length)) {
      throw new Error('TransportGUN.p_rawadd args passed bad arguments:');
    }
    this.connection(url)
  ...
  async p_newlisturls(cl) {
    const u = await this._p_newgun(cl);
    return [u, u];
  }
  ...
  async _p_newgun(pubkey) {
    if (pubkey.hasOwnProperty('keypair')) pubkey = pubkey.keypair.signingexport();
  ...
  async p_newdatabase(pubkey) {
    ...
    */
    const u = await this._p_newgun(pubkey);
  ...
  async p_newtable(pubkey, table) {
    ...
    */
    ...
    const database = await this.p_newdatabase(pubkey);
  ...
  async p_set(url, keyvalues, value) {  // url = yjs:/yjs/database/table
     ...
     */
    const table = this.connection(url);
    if (typeof keyvalues === 'string') {
      ...
      if (Array.isArray(keyvalues)) {
        throw new Error('TransportGUN - should not be passing an array as the keyvalues');
      }
      table.put(
        Object.keys(keyvalues).reduce(
          (previous, key) => { previous[key] = canonicaljson.stringify(keyvalues[key]); return previous; },
          {}
        )
      );
    }
  ...
  async p_get(url, keys) {
    ...
    if (Array.isArray(keys)) {
      ...
      */
      ...
      const val = await this._p_once(table.get(keys));    // Resolves to value
  ...
  async p_keys(url) {
    const res = await this._p_once(this.connection(url));
  ...
  async p_getall(url) {
    ...
    return Object.keys(res)
      .filter(k => (k !== '_') && res[k] !== null) // See WORKAROUND-GUN-UNDERSCORE and WORKAROUND-GUN-DELETE
      .reduce((previous, key) => { previous[key] = JSON.parse(res[key]); return previous; }, {});
  }
